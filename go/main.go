...
func main() {
	...
	db, err := initStore()
	if err != nil {
		log.Fatalf("failed to init the store: %s\n", err)
	}
	defer db.Close()

	// http://localhost:servicePort/ 获取消息数量
	e.GET("/", func(c echo.Context) error {
		// handler function
		return rootHandler(db, c)
	})

	// http://localhost:servicePort/ping 获取服务状态
	e.GET("/ping", func(c echo.Context) error {
		return c.JSON(http.StatusOK, struct{ Status string }{Status: "OK"})
	})

	// POST http://localhost:servicePort/send 发送message
